...
def main():
    ...
    ap.add_argument("--aspects", default="all", help="majors | majors+quintiles | all | harmonic12 | h<N> (one harmonic family only, e.g. h7 = septile, biseptile, triseptile)")
    ...
    zodiac = "tropical" if a.carving == "equal" else a.carving
    family = re.fullmatch(r"h(\d+)", a.aspects)
    astro = charts.sky_post("/sky/astrology", birth, houses=hsys, ayanamsa=a.ayanamsa, zodiac=zodiac,
                            aspect_set="harmonic12" if (a.aspects == "harmonic12" or family) else "majors")
    if astro is None:
        sys.exit("astrology lookup failed")
    ...
    if family:
        want = set(_AspectTables.HARMONIC_NAMES.get(int(family.group(1)), []))
    else:
