...
// exposes one tool, comms_reply, which writes spool/outbox/<event_id>.json for the
...
Do the work in this environment, then ALWAYS finish by calling comms_reply with the event_id:
...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: "comms_reply",
      description: "Send the reply for a comms request back to the user's calendar (read aloud on the phone).",
      inputSchema: {
        type: "object",
        properties: {
...
mcp.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  if (!event_id || !["done", "question"].includes(status) || !text) {
    return { content: [{ type: "text", text: "comms_reply needs event_id, status (done|question) and text" }], isError: true };
  }
...
async function deliver(file) {
  ...
  lines.push("", `Reply with comms_reply(event_id="${req.event_id}", status="done"|"question", text=...).`);
