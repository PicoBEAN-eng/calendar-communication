...
```
phone voice ──creates event──▶ "Woolly" calendar ◀──patches ⏳ ✓ ? + brief── relay_poller.py
                                                                             │ spool/inbox   ▲ spool/outbox
                                                                             ▼               │
                                                     inbox session ◀── relay_channel.mjs (route 2)
                                                     "Woolly inbox"     or relay_send_p.py  (route 3)
                                                          ▲
...
| file | side | role |
|---|---|---|
| `relay_poller.py` | Google | sync-token poll, claim new events (⏳), write `spool/inbox/*.json`, push `spool/outbox/*.json` replies (✓ / ?, brief, buzz) |
| `relay_channel.mjs` | Claude | **route 2**: MCP channel server run by the session; pushes inbox files in as `<channel>` events, exposes `relay_reply` |
| `relay_send_p.py` | Claude | **route 3** fallback: `claude -p` + SendMessage per inbox file |
| `bin/relay-reply` | Claude | write an outbox reply from a shell (route 3 or by hand) |
...
| `relay.toml` | config | calendar id, stream, session name/cwd/mode, route flags |
...
3. **This box**:
   ```bash
   cp relay.toml.example relay.toml           # edit stream / session_name / session_cwd
   mkdir -p state && cp ~/Downloads/client_secret_*.json state/client_secret.json
   ../woolly-workplace/.venv/bin/python relay_poller.py --auth      # paste-the-URL consent
   ../woolly-workplace/.venv/bin/python relay_poller.py --list-calendars   # → calendar_id
   ../woolly-workplace/.venv/bin/python relay_poller.py --once
   ```
...
4. **Route 2 (channel shim)**: nothing to register — `bin/inbox-session` writes the MCP
   ...
   the session inside tmux session `relay-inbox` and answers the development-channels
   consent dialog itself (that dialog appears at *every* start and is not persisted, which
   is also why route 2 cannot run under `claude -p`). `tmux attach -t relay-inbox` to watch.
   **Route 3** instead: set `channel_flags = ""` in `relay.toml` and uncomment the
   `ExecStartPost` line in the poller service.
5. **Units**: `cp deploy/* ~/.config/systemd/user/ && systemctl --user daemon-reload &&
   systemctl --user enable --now nexus-relay-poller.timer nexus-relay-inbox.service`.
6. **Phone**: in the inbox session run `/config` and turn on *Push when Claude decides* and
   *Push when actions required* (permission prompts then reach the phone). In the Claude
   ...
   > I have Google calendars named "Woolly" and "Frames" that relay requests to agents.
   ...
   > relay", look at those calendars for events in the last 24 h whose title starts with
...
1. `relay_poller.py --once` claims a hand-made event (⏳ appears on the phone).
...
3. Inbox session in the foreground with the channel flag: drop a file in `spool/inbox/`,
   watch the `<channel>` event land, see `relay_reply` write the outbox, poller pushes ✓,
...
- **route 2 end to end in a real interactive session**: `Channel notifications registered`,
  the spool file arrived as `← relay: Relay request…`, Claude called `relay_reply`, outbox
