#!/usr/bin/env node
// Drive relay_channel.mjs over stdio as an MCP client (no Claude involved):
// expects the tool list, a channel notification for a dropped inbox file, and an outbox
// file after calling relay_reply.  Usage: node test/channel_smoke.mjs
...
const spool = fs.mkdtempSync(path.join(os.tmpdir(), "relay-spool-"));
fs.mkdirSync(path.join(spool, "inbox"), { recursive: true });

const transport = new StdioClientTransport({
  command: "node",
  args: [path.join(here, "..", "relay_channel.mjs")],
  env: { ...process.env, RELAY_SPOOL: spool, RELAY_STREAM: "Smoke" },
  stderr: "pipe",
});
...
const res = await client.callTool({ name: "relay_reply", arguments: { event_id: "evt123", status: "done", text: "Hello back." } });
console.log("relay_reply ->", res.content[0].text);
